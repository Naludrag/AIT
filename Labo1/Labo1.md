...
For the zip command we did the same procedure. Except that this time when we restored the directory, it preserved the permissions, owner and latest modification time. In this case the restore isn't "really" done. The metadata remains the same.
