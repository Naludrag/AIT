...
#### 1.2
It should keep the same session id even if the user refresh the page. For that, we should not speak with a different server, but with the same that created the session. We would like that because, the server that generated the session can validate the token given and so keep the right session. We should add Session Stickyness or use a different algorithm than roundrobin to have this behavior.
